...
class WechatBasic(WechatBase):
    ...
    def _upload_media_py3(self, media_type, media_file, extension=''):
        if isinstance(media_file, io.IOBase) and hasattr(media_file, 'name'):
            ...
            if not is_allowed_extension(extension):
                raise ValueError(
                    'Please provide \'extension\' parameters when the type of \'media_file\' is \'io.BytesIO\'.')
            filename = 'temp.' + extension
        else:
            raise ValueError(
                'Parameter media_file must be io.BufferedIOBase(open a file with \'rb\') or io.BytesIO object.')
